...
// import { Card, CardHeader, CardTitle } from "../components/ui/card"; // Fixed: Removed unused CardContent
...
//   // Fixed: Use 'alert' param to resolve unused warning (e.g., log or use in nav)
//   const handleViewAlert = (alert: Alert) => {
//     // Example usage: Log or pass to URL if needed
//     console.log("Viewing alert for document:", alert.name);
...
//                 key={doc._id}
//                 document={doc}
//                 canEditDocuments={isAdminUser || doc.uploadedBy === user?._id}
...
import { Badge } from "../components/ui/badge";
import { toast } from "sonner";
import type { Document, Alert } from "../types";
...
const OrganizationDocumentsPage = () => {
  ...
  const getAlertDisplay = (alert: Alert) => {
    let message = "";
    let subText = "";
    let iconVariant: "destructive" | "warning" | "secondary" = "secondary";
    ...
    if (alert.alertType === "expiry") {
      ...
      if (alert.flagColor === "red") {
        iconVariant = "destructive";
        borderClass = "border-destructive";
      } else if (alert.flagColor === "orange") {
        iconVariant = "warning";
  ...
  const handleViewAlert = (alert: Alert) => {
    console.log("Viewing alert:", alert.name);
    setShowAlerts(false);
  };
                    ...
                    {alerts.length === 0 ? (
                      ...
                      alerts.map((alert) => {
                        ...
                        return (
                          <div
                            key={alert._id}
                            className={`bg-background rounded-lg p-3 cursor-pointer transition-all hover:bg-muted border ${borderClass}/30 shadow-sm`}
                            onClick={() => handleViewAlert(alert)}
                          >
                            <div className="flex items-start gap-3">
                              <AlertTriangle
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {filteredDocuments.map((doc: Document) => (
              <DocumentCard
                ...
                document={doc}
                canEditDocuments={isAdminUser || doc.uploadedBy === user?._id}
                canDeleteDocuments={isAdminUser || doc.uploadedBy === user?._id}
                onView={() => handleDocumentAction("view", doc)}
                onDownload={() => handleDocumentAction("download", doc)}
                onDelete={() => handleDocumentAction("delete", doc)}
              />
