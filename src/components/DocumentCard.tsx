  ...
  const handleUpdateDocument = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (expiryDateStr && expiryDateStr !== "") {
        // Basic validation: check if it's a valid date string (YYYY-MM-DD)
        const date = new Date(expiryDateStr);
        if (
          isNaN(date.getTime()) ||
          date.toISOString().split("T")[0] !== expiryDateStr
        ) {
        ...
        <DialogContent className="sm:max-w-sm">
          <DialogHeader className="space-y-1">
            ...
            <DialogDescription className="text-xs">
              Update the name, type, negotiated amount, and expiry date of the
              document.
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={handleUpdateDocument} className="space-y-3">
            ...
            <div className="space-y-1">
              ...
              <Input
                ...
                name="expiryDate"
                type="date"
                defaultValue={formatDateForInput(editingDocument?.expiryDate)}
                disabled={isUpdating}
                className="h-8 text-xs"
              />
            </div>
