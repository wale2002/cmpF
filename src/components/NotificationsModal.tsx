...
// import { Bell, AlertTriangle, BarChart3, Activity, Users } from "lucide-react";
...
//   const [activeTab, setActiveTab] = useState<"notifications" | "alerts" | "analytics">("notifications");
...
//   // NEW: Analytics query (assuming endpoint exists; fallback to local computation)
//   const { data: notificationsAnalytics } = useQuery({
//     queryKey: ["notificationsAnalytics", user?.organization],
//     queryFn: () => documentService.getNotificationsAnalytics(user?.organization || ""),
//     enabled: !!user?.organization,
//   });
...
//   // Local fallback analytics computation
//   const computedAnalytics = (() => {
//     const totalNotifications = notifications.length;
//     const unread = notifications.filter(n => !n.read).length;
//     const read = totalNotifications - unread;
//     const recent7Days = notifications.filter(n => {
//       const now = new Date("2025-10-25T00:00:00Z"); // Updated to current date
//       const created = new Date(n.createdAt);
//       return (now.getTime() - created.getTime()) / (1000 * 60 * 60 * 24) <= 7;
//     }).length;

//     // Notification types breakdown (assuming 'type' field; fallback to message keywords)
//     const typeBreakdown = notifications.reduce((acc, n) => {
//       const type = n.type || (n.message.includes('upload') ? 'Upload' : n.message.includes('expiry') ? 'Expiry' : 'Other');
...
//   const analyticsData = notificationsAnalytics?.data || computedAnalytics;
...
//         </Button>
//       </DialogTrigger>
//       <DialogContent className="max-w-4xl max-h-[85vh] overflow-y-auto"> {/* Wider for analytics */}
//         <DialogHeader>
//           <DialogTitle>Notifications & Alerts</DialogTitle>
//           <DialogDescription>
//             Stay updated with document activities, expiry reminders, and insights
//           </DialogDescription>
//         </DialogHeader>
//         <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as "notifications" | "alerts" | "analytics")}>
...
//               </div>
//               <div className="flex flex-col items-center p-4 bg-muted rounded-lg">
//                 <Bell className="h-8 w-8 text-destructive mb-2" />
//                 <div className="text-2xl font-bold text-destructive">{analyticsData.unread}</div>
//                 <div className="text-sm text-muted-foreground">Unread</div>
//               </div>
//               <div className="flex flex-col items-center p-4 bg-muted rounded-lg">
//                 <Users className="h-8 w-8 text-secondary mb-2" />
//                 <div className="text-2xl font-bold text-secondary">{analyticsData.recent7Days}</div>
...
//                 <div className="h-64">
//                   {/* Placeholder for Recharts Pie; import if needed */}
//                   <div className="flex items-center justify-center h-full text-muted-foreground">
//                     Read: {analyticsData.read} | Unread: {analyticsData.unread}
//                   </div>
//                   {/* To add: <PieChart><Pie data={[{name: 'Read', value: analyticsData.read}, {name: 'Unread', value: analyticsData.unread}]} /></PieChart> */}
//                 </div>
//               </div>
//               {/* Type Breakdown Bar */}
//               <div className="bg-background rounded-lg p-6 border">
...
//                       <div className="flex items-center gap-2">
//                         <div className="w-20 bg-primary/20 h-2 rounded-full">
//                           <div
...
//               Analytics updated {formatRelativeTime(notificationsData?.data?.updatedAt || new Date().toISOString())}
//             </div>
//           </TabsContent>
//         </Tabs>
//       </DialogContent>
