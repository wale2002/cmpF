...
const DashboardStats = ({
  ...
  const totalUsers = userMetricsData?.data?.metrics?.totalUsers || 0;
  const totalOrganizations =
    orgMetricsData?.data?.metrics?.totalOrganizations || 0;

  const userMetricsLoading = !!user && canViewUsers && !userMetricsData;
  const orgMetricsLoading = !!user && canViewOrganizations && !orgMetricsData;

  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
      <Card className="w-full">
        <CardHeader className="flex flex-col sm:flex-row items-start sm:items-center justify-between space-y-2 sm:space-y-0 pb-2">
          <CardTitle className="text-xs sm:text-sm font-medium text-left">
            Total Documents
          </CardTitle>
          <FileText className="h-4 w-4 text-muted-foreground mt-1 sm:mt-0" />
        </CardHeader>
        ...
        <Card className="w-full">
          ...
          <CardContent className="pt-0">
            <div className="text-xl sm:text-2xl font-bold">
              {orgMetricsLoading ? "..." : totalOrganizations}
            </div>
            <p className="text-xs sm:text-sm text-muted-foreground mt-1">
              Total organizations
            </p>
