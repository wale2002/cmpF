...
type UsersResponse = ApiResponse<{ users: User[]; total: number; page: number; totalPages: number; }>;
...
export const UserManagement = () => {
  ...
  const { data: usersResponse, isLoading, error, refetch } = useQuery<UsersResponse, Error>({
    queryKey: ["allUsers"],
    queryFn: () => userService.getAllUsers(),
    enabled: true,
  ...
  const deleteMutation = useMutation({
    ...
    onError: (error: Error) => { // FIXED: Explicit Error type
      handleApiError(error, "Failed to delete user");
    },
  });
